...
class SGNHeadOneV2(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if CONF.TPV.USE_V2 or CONF.TPV.USE_V3:
            scale = 4
        else:
            scale = 2
        ...
        if not CONF.TPV.USE_V2 and not CONF.TPV.USE_V3:
        ...
        self.occ_header = nn.Sequential(
            SDB(channel=self.embed_dims, out_channel=self.embed_dims//scale, depth=1),
            nn.Conv3d(self.embed_dims//scale, 1, kernel_size=3, padding=1)
        )
        ...
        self.sdb = SDB(channel=self.embed_dims+occ_channel, out_channel=self.embed_dims//scale, depth=depth)
        self.ssc_header = Header(self.n_classes, feature=self.embed_dims//scale)
    ...
    def forward(self, mlvl_feats, img_metas, target):
        ...
        elif CONF.TPV.USE_V3:
            x3d = x3d + img_metas[0]['latent_feats']
