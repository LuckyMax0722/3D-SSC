...
if CONF.LATENTNET.USE_V5:
    ...
    from .latent_v5 import vqvae_2 as vqvae
    from .latent_v5 import mlp_unet
    ...
    class LatentNet(nn.Module):
        def __init__(self):
            ...
            self.encoder_x = mlp_unet(
                ...
                out_channel=CONF.SGN.embed_dims // 2, # 64 
                feature=CONF.SGN.embed_dims // 2,  # 64
                class_num=CONF.SSD.num_classes, # 20
            )

            self.class_num = CONF.SSD.num_classes # 21
            self.l_size = CONF.SSD.l_size
            self.fc1 = nn.Linear(CONF.SSD.out_dim, CONF.SSD.out_dim)
            self.fc2 = nn.Linear(CONF.SSD.out_dim, CONF.SSD.out_dim)
        ...
        def forward_train(self, x3d, target):
            ...
            target_20 = torch.where(target == 255, 20, target.long())
            
            latent_post = self.encoder_y.forward_encoder(target_20)  # --> torch.Size([1, 21, 32, 32, 4])

            if self.l_size == '32322':
                latent_post = latent_post.view(1, self.class_num, 32 * 32 * 8)
            elif self.l_size== '16162':
                latent_post = latent_post.view(1, self.class_num, 16 * 16 * 4)
            elif self.l_size== '882':
                latent_post = latent_post.view(1, self.class_num, 8 * 8 * 2)

            latent_post = latent_post[0]  # 

            # 2. Input Branch
            latent_prior = self.encoder_x(x3d)  # --> torch.Size([21, 1024])

            # 3. KL Branch
            mu_y, logvar_y, dist_post = self.dist(latent_post)  # torch.Size([21, 1024]) -->
            mu_x, logvar_x, dist_prior = self.dist(latent_prior)     
            ...
            # 5 Reconstruction
            z_noise_post = self.reparametrize(mu_y, logvar_y)  # --> torch.Size([21, 1024]) 
            z_noise_post = z_noise_post.unsqueeze(0)  # torch.Size([21, 1024]) --> torch.Size([1, 20, 1024])
            
            # torch.Size([1, 21, 1024]) --> torch.Size([1, 21, 16, 16, 4])
            if self.l_size == '32322':
                z_noise_post = z_noise_post.view(1, self.class_num, 32, 32, 8)
            elif self.l_size== '16162':
                z_noise_post = z_noise_post.view(1, self.class_num, 16, 16, 4)
            elif self.l_size== '882':
                z_noise_post = z_noise_post.view(1, self.class_num, 8, 8, 2)
        ...
        def forward_test(self, x3d):
            ...
            # torch.Size([1, 20, 1024]) --> torch.Size([1, 20, 16, 16, 4])
            if self.l_size == '32322':
                z_noise_prior = z_noise_prior.view(1, self.class_num, 32, 32, 8)
            elif self.l_size== '16162':
                z_noise_prior = z_noise_prior.view(1, self.class_num, 16, 16, 4)
            elif self.l_size== '882':
                z_noise_prior = z_noise_prior.view(1, self.class_num, 8, 8, 2)
