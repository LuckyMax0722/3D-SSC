...
class HeaderV5(nn.Module):
    ...
    def forward(self, x3d_l1, recons_logit):
        ...
        ssc_logit = ssc_logit_full.reshape(w, l, h, self.class_num).permute(3,0,1,2).unsqueeze(0)  # --> torch.Size([1, 20, 256, 256, 32])
        
        recons_logit = self.up_scale_2(recons_logit)  # --> torch.Size([1, 20, 256, 256, 32])
        
        ssc_logit = ssc_logit * recons_logit  # --> torch.Size([1, 20, 256, 256, 32])
