...
class Image2Depth:
    def __init__(self, dataset, datapath, testlist, savepath, num_seq, baseline, model):
    ...
    def test(self):
        ...
        for _, sample in tqdm(enumerate(self.TestImgLoader), total=len(self.TestImgLoader), desc=f"Processing sequence {self.num_seq}"):
...
if __name__ == '__main__':
    ...
    model = 'MSNet3D'
    maxdisp = 192
    loadckpt = CONF.PATH.CHECKPOINT_MSN3D
    ...
    for num_seq, baseline in sequences.items():
        datapath = os.path.join(CONF.PATH.DATA_DATASETS_SEQUENCES, num_seq)
        testlist = os.path.join(CONF.PATH.MOBILESTEREONET, 'filenames', num_seq + '.txt')
